"""Extend data/caption_matched/manifest.csv with the 60 SDXL rows (once generated), reusing exactly the same
...
for r in real.itertuples():
    p = Path("data/caption_matched/sdxl") / f"{r.content_id}.png"
    if not p.exists():
        continue
