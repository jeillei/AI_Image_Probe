...
## path_length mechanism decomposition — the final feature-level mechanism experiment (E115)

*Scope: **exploratory/mechanistic stopping-point experiment, not an independent preregistered confirmation** —
the project's final planned feature-level mechanism decomposition, directly resolving E114's open question
(SDXL passed its trajectory-stage test, PixArt did not, yet curvature is equally VAE-redundant for both). Plan
frozen in `ANALYSIS_PLAN_PATH_LENGTH_MECHANISM.md` before any result below was inspected. Full results:
`PATH_LENGTH_MECHANISM.md`. No new generation, no new feature extraction, no HPC — `C_resid` reused verbatim
from E114, not recomputed.*

| exp | question | type | design | data | result | interpretation |
|---|---|---|---|---|---|---|
| **E115 path_length VAE-residualization + SDXL/PixArt feature-addition decomposition + interaction test + transfer ablation** | does `path_length`, alone or jointly with `diffpath_curvature`, explain why the trajectory stage added information for SDXL but not PixArt | **exploratory (mechanistic stopping-point experiment)** | identical residualization machinery applied to `path_length` (label-blind, cross-fitted `Ridge(alpha=1.0)` on VAE features); direct feature-addition decomposition (VAE / VAE+curvature / VAE+path_length / VAE+both, with and without score) for SDXL and PixArt specifically; one preregistered `curvature×path_length` interaction term; PixArt→aMUSEd transfer ablation removing `path_length` and the full trajectory stage | same 60 content ids × 4 generators, already-cached v2 panel | **Path_length's VAE-redundancy pattern is the *inverse* of curvature's.** SD1.5/SDXL's residual path_length effect *strengthens* after VAE-residualization (173%/186% retention, both CIs excluding zero, SDXL alone reaching AUROC 0.695) while PixArt's raw positive effect (d=+0.38) reverses sign and becomes non-significant (d=−0.17, CI crosses zero) — aMUSEd's very large raw effect (d=+1.39) collapses almost entirely (5.3% retention). The central feature-addition decomposition **exactly reproduces both generators' original preregistered numbers** (SDXL VAE+score+curvature+path_length vs VAE+score: +0.053 [0.016,0.092]; PixArt: −0.005 [−0.029,0.018]) and shows `path_length` alone accounts for essentially all of SDXL's gain (+0.052 of the +0.053) while curvature's own marginal contribution is null everywhere for both generators; PixArt's ΔAUROC is significantly *negative* when either feature is added to VAE alone. The one preregistered interaction term (`curvature×path_length`) adds nothing for any generator on any metric (all CIs include zero), ruling out P2/P3. PixArt→aMUSEd transfer ablation: removing `path_length` barely changes transfer (0.9486→0.9495); removing the whole trajectory stage *increases* it (→0.9765) — `path_length` does not materially contribute to that transfer | **Outcome P1 (path length explains SDXL vs PixArt)** — the cleanest of the four preregistered outcomes, with both prior primary-test numbers reproduced under decomposition. Recommendation: **mechanism exploration stops here**; the next experiment is the final robustness/controlled-AI-edit validation phase (testing whether `path_length`'s newly-established role survives transformation/edit pilots for SD1.5/SDXL specifically), not a further feature search |
