...
* **Update (feature-level audit, `FEATURE_AUDIT.md`, E88): the overfitting question above is now resolved, not just flagged.** A pre-specified 66-column subset located by that audit (`rich_score_spatial`+`rich_score_fft`, the score map's spatial autocorrelation and frequency spectrum) reproduces the aMUSEd increment almost exactly (+0.064 vs E86's +0.081) and is carried by 1–2 features individually reaching held-out AUROC 0.86–0.98 with bootstrap sign-stability 1.000 — i.e. it is a real, concentrated, reproducible effect, **not** high-dimensional overfitting. But it is also disjoint from what separates SD1.5 (0% top-10/25 feature overlap between tasks; SD1.5's own top features run on the guidance family instead, individually near-chance and diffuse) and is best read as a generic VQGAN-decoder texture/frequency artifact — consistent with a bare VAE reconstruction-error baseline (0.87) and a DiT probe sharing SD1.5's VAE family (0.94) also separating aMUSEd strongly while a colour-only statistic does not (0.68). Locating the mechanism sharpens, rather than reopens, the negative conclusion: aMUSEd is separable by an identifiable non-provenance artifact, not by the same (or any generalizing) mechanism that separates SD1.5.
* This is not "weakened" (a term implying residual, defensible uncertainty in favour of the hypothesis) because the specific residual carried forward from the prior audit — "does a non-SD generator show the same signal via the same mechanism" — was tested directly and came back negative on both counts that matter (transfer, and information beyond baselines in interpretable form), and the feature audit now positively identifies *why* (disjoint mechanisms) rather than leaving it an open question. It is not "unresolved" because the pre-registered decision rules were followed exactly, gave a clean, non-contradictory (if unflattering) answer per the addendum's own table, and the one loose end (was the aMUSEd increment an overfitting artifact?) has since been closed against the hypothesis, not left open. It is **unsupported**: the evidence collected under the project's own rules does not support the claim that SD1.5 inverse trajectories carry a generator-general provenance signal.
